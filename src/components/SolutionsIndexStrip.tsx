// src/components/SolutionsIndexStrip.tsx
import { SERVICE_CATALOG } from "@/lib/siteStatus";

export default function SolutionsIndexStrip() {
  return (
    <div className="overflow-hidden rounded-3xl border border-white/10 bg-black/25">
      <div className="relative h-px">
        <div className="absolute inset-0 bg-white/10" />
        <div className="orbit-slide absolute -left-24 top-0 h-px w-56 bg-gradient-to-r from-transparent via-[#FACC15]/35 to-transparent" />
      </div>

      <div className="p-5 sm:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <div className="text-[11px] tracking-[0.28em] text-white/55">
              SERVICE INDEX
            </div>
            <div className="mt-2 text-sm text-white/80">
              Business services, controlled onboarding, and buyer-ready navigation
            </div>
          </div>

          <div className="flex items-center gap-2 text-xs text-white/55">
            <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5">
              {SERVICE_CATALOG.length} services
            </span>
            <span className="rounded-full border border-[#FACC15]/25 bg-[#FACC15]/10 px-3 py-1.5 text-[#FDE68A]">
              premium service index
            </span>
          </div>
        </div>

        <div className="mt-4 flex gap-2 overflow-x-auto pb-1 no-scrollbar">
          {SERVICE_CATALOG.map((service) => (
            <a
              key={service.id}
              href={`#${service.id}`}
              className="shrink-0 rounded-full border border-white/10 bg-white/5 px-4 py-2 text-[13px] text-white/80 transition hover:bg-white/10"
            >
              {service.publicLabel}
            </a>
          ))}
        </div>
      </div>
    </div>
  );
}